      ...
      <form @submit.prevent="handleSubmit" class="space-y-8">
        ...
        <div class="bg-white p-8 md:p-10 rounded-[40px] shadow-xl shadow-blue-900/5 border border-slate-100">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-3  ">
              <label class="text-xs font-black uppercase tracking-[0.2em] text-slate-400 ml-1">City, Region (Cameroon)</label>
              <div class="relative group">
                ...
                <select
                    ...
                    class="w-full pl-14 pr-12 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none font-bold text-slate-900 appearance-none cursor-pointer"
                    required
                >
                  <option value="" disabled>Select a location</option>
                  <option
                      v-for="loc in CAMEROON_LOCATIONS"
                      :key="loc.city"
                      :value="{ city: loc.city, region: loc.region, neighborhood: job.location.neighborhood }"
                  >
                    {{ loc.city }}, {{ loc.region }}
                  </option>
                </select>
            ...
            <div class="space-y-3 ">
              <label class="text-xs font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Neighbourhood</label>
              <div class="relative group ">
                <Mountain class="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-300 group-focus-within:text-blue-500 transition-colors" />
                <input
                    v-model="job.location.neighborhood"
                    type="text"
                    class="w-full pl-14 pr-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none font-bold text-slate-900 placeholder:text-slate-300"
                    placeholder="Quartier (ex: Bastos, Akwa...)"
                    required
                />
              </div>
            </div>
